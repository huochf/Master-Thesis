...
from hoi_recon.utils.optim_losses import SMPLPostPriorLoss, SMPLKpsProjLoss, ObjectEproPnpLoss, MultiViewPseudoKps2DLoss

from .train_c_flow_pseudo_kps2d import Model
...
def evaluate_framewise_on_behave(cfg):
    ...
    metadata = BEHAVEExtendMetaData(cfg.dataset_root_dir)
    all_sequences = metadata.get_all_image_by_sequence(split='test')

    flow_dim = (22 + metadata.object_num_keypoints[args.object] + 1) * 3
    ...
    output_dir = 'outputs/cflow_pseudo_kps2d/'
    ...
    for idx, batch in enumerate(tqdm(test_dataloader)):
        ...
        loss_functions = [
            ...
            MultiViewPseudoKps2DLoss(model, hoi_feats, n_obj_kps=metadata.object_num_keypoints[args.object], n_views=args.cam).to(device),
        ]
        param_dicts = [
        ...
        parameters = [hoi_instance.smpl_betas, hoi_instance.smpl_body_pose6d, hoi_instance.obj_rel_trans, hoi_instance.obj_rel_rot6d,
            hoi_instance.hoi_trans, hoi_instance.hoi_rot6d, loss_functions[-1].cam_pos]
        ...
        loss_weights = {
            ...
            'loss_kps_nll': lambda cst, it: 10. ** 0 * cst / (1 + 10 * it),
        }
        itertations = 2
        steps_per_iter = 500
    ...
    save_pickle(recon_results, os.path.join(output_dir, 'recon_results_framewise_optim_cam_{:02d}_{}.pkl'.format(args.cam, args.object)))
    ...
    save_json(evaluate_results, os.path.join(output_dir, 'evaluate_results_framewise_optim_cam_{:02d}_{}.json'.format(args.cam, args.object)))
