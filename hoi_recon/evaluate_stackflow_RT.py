...
from hoi_recon.utils.optim_losses import SMPLPostPriorLoss, SMPLKpsProjLoss, ObjectEproPnpLoss, HORTLoss
...
def evaluate_framewise_on_behave(cfg):
    ...
    for idx, batch in enumerate(tqdm(test_dataloader)):
        ...
        loss_functions = [
            ...
            HORTLoss(stackflow.stackflow, hoi_feats, object_labels).to(device),
        ]
        param_dicts = [
    ...
    save_pickle(recon_results, os.path.join(output_dir, 'recon_results_framewise.pkl'))
    ...
    save_json(evaluate_results, os.path.join(output_dir, 'evaluate_results_framewise.json'))
