...
from hoi_recon.utils.optim_losses import SMPLPostPriorLoss, SMPLKpsProjLoss, ObjectEproPnpLoss, TransKps3DLoss, TransMultiViewPseudoKps2DLoss
...
def evaluate_framewise_on_behave(cfg):
    ...
    for idx, batch in enumerate(tqdm(test_dataloader)):
        ...
        loss_functions = [
            ...
            # TransMultiViewPseudoKps2DLoss(model, hoi_feats, n_obj_kps=metadata.object_num_keypoints[args.object], n_views=8).to(device),
            TransKps3DLoss(model, hoi_feats, n_obj_kps=metadata.object_num_keypoints[args.object]).to(device),
        ]
        # param_dicts = [
    ...
    # save_pickle(recon_results, os.path.join(output_dir, 'recon_results_framewise_optim_prior_cam8.pkl'))
    ...
    # save_json(evaluate_results, os.path.join(output_dir, 'evaluate_results_framewise_optim_prior_cam8.json'))
