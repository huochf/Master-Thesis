...
from hoi_recon.datasets.behave_kps3d_img_dataset_objectwise import BEHAVE3DKpsDataset
...
class Model(nn.Module):
    ...
    def sampling(self, n_samples, visual_feats, z_std=1.):
        visual_condition = self.visual_embedding(visual_feats)
        conditions = visual_condition + object_condition
...
def train(args):
    ...
    dataset_train = BEHAVE3DKpsDataset(args.root_dir, args.object, split='train')
    dataset_test = BEHAVE3DKpsDataset(args.root_dir, args.object, split='test')
    dataloader_train = torch.utils.data.DataLoader(dataset_train, batch_size=batch_size, num_workers=16, shuffle=True)
    ...
    for epoch in range(begin_epoch, args.epoch):
        ...
        for idx, item in enumerate(dataloader_train):
            img_ids, images, hoi_kps = item
            images = images.float().to(device)
            hoi_kps = hoi_kps.float().to(device)

            batch_size, n, _ = hoi_kps.shape
            hoi_kps = hoi_kps.reshape(batch_size, -1)

            hoi_kps = hoi_kps + 0.001 * torch.randn_like(hoi_kps)

            visual_feats = model.image_embedding(images).reshape(batch_size, -1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', default='./outputs/cglow_kps3d')
