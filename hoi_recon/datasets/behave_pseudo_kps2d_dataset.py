...
class BEHAVEPseudoKps2DDataset:

    def __init__(self, root_dir, object_name='backpack', n_views=8, use_gt_grouping=False, split='train'):
        ...
        if not self.use_gt_grouping:
            self.knn_grouping = load_pickle('outputs/behave_knn_grouping/knn_groups_{}_k_08.pkl'.format(object_name))

        self.metadata = BEHAVEExtendMetaData(root_dir)

        self.hoi_kps_seq_all, self.hoi_bb_xyxy = self.load_kps()
        self.item_ids = list(self.hoi_kps_seq_all.keys())
        ...
        for file in os.listdir(datalist_dir):
            ...
            for cam_id in annotation_list:
                for idx, item in enumerate(annotation_list[cam_id]):
                    hoi_kps_seq_all[item['img_id']] = self.kps3dfy(item)
                    hoi_bboxes[item['img_id']] = item['hoi_bb_xyxy']

        return hoi_kps_seq_all, hoi_bboxes
    ...
    def load_item_kps(self, item_id):
        if self.use_gt_grouping:
            day_id, sub_id, obj_name, inter_type, frame_id, cam_id = item_id.split('_')

            hoi_kps = []
            for i in range(self.n_views):
                cam_id = str(np.random.randint(4))
                item_id = '_'.join([day_id, sub_id, obj_name, inter_type, frame_id, cam_id])
                hoi_kps.append(self.hoi_kps_seq_all[item_id])
            hoi_kps = np.stack(hoi_kps, axis=0)
        else:
    ...
    def get_item(self, idx):
        ...
        image_path = self.metadata.get_image_path(item_id)
