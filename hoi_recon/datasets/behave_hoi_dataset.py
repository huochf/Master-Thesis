...
from hoi_recon.datasets.utils import (
    load_pickle, 
    load_json,
    get_augmentation_params_from_cfg, 
...
class BEHAVEExtendDataset(object):

    def __init__(self, cfg, annotations=None, object_name='all', is_train=True, for_evaluation=False):
        self.cfg = cfg
        ...
        if annotations is None:
            self.annotations = self.load_annotations(cfg)
        else:
        ...
        if object_name != 'all':
            ...
            for item in  self.annotations:
                if item['img_id'].split('_')[2] == object_name:
                    new_annotation_list.append(item)
            self.annotations = new_annotation_list
    ...
    def load_annotations(self, cfg):
        annotations_all = {}
        seq_names = list(self.dataset_metadata.go_through_all_sequences(split=self.split))
    ...
    def change_bg(self, image, img_id):
        ...
        object_full_mask_path = self.dataset_metadata.get_object_full_mask_path(img_id)
    ...
    def get_item_for_training(self, idx):
        ...
        image_path = self.dataset_metadata.get_image_path(img_id)
        ...
        if self.is_train:
            tx, ty, rot, scale, color_scale = get_augmentation_params_from_cfg(self.cfg)
        else:
        ...
        smpl_trans_3d = annotation['smplh_joints_3d'][0].copy()
