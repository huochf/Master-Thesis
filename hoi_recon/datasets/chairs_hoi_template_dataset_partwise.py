...
class CHAIRSDataset:
    ...
    def __init__(self, root_dir, res=64, split='train'):
        ...
        self.object_verts_all, self.object_verts_num = self.load_object_meshes()
    ...
    def load_object_meshes(self, ):
        ...
        object_verts_num = np.zeros((NUM_OBJECT, OBJECT_PARTS_NUM))
        ...
        for object_id in os.listdir(object_mesh_dir):
            ...
            for file in os.listdir(os.path.join(object_mesh_dir, object_id)):
                ...
                object_verts_num[object_ind_in_meta, part_idx] = MESH_VERTICES_NUM_MAX

        return object_verts_all, object_verts_num
    ...
    def __getitem__(self, idx):
        ...
        object_v_len = self.object_verts_num[object_ind_in_meta] # [7, ]

        for _i in range(OBJECT_PARTS_NUM):
            if object_v_len[_i] == 0:
                object_R[_i] = np.eye(3)
                object_T[_i] = np.zeros(3)

            if np.isnan(object_T[_i]).any():
                object_R[_i] = np.eye(3)
                object_T[_i] = np.zeros(3)
                object_v[_i] = 0
                object_v_len[_i] = 0
