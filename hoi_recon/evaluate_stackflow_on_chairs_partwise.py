...
class ImageDataset:
    ...
    def collect_item_ids(self, ):
        ...
        for img_id in self.img_ids:
            ...
            object_id = img_id.split('_')[0]
            for part_id in np.nonzero(self.vertex_len[int(object_id)])[0]:
                if not np.isnan(annotation['object_rel_rotmat'][part_id]).any():
                    item_ids.append((img_id, part_id))
        return item_ids
    ...
    def load_meshes(self, ):
        ...
        object_inter_mesh_dir = 'data/datasets/chairs/object_inter_shapes_alpha_100'
        object_inter_meshes = {}
    ...
    def __getitem__(self, idx):
        img_id, part_id = self.item_ids[idx]
        annotation = self.annotations[img_id]
        ...
        results['object_rel_rotmat'] = annotation['object_rel_rotmat'][part_id].astype(np.float32)
        results['object_rel_trans'] = annotation['object_rel_trans'][part_id].astype(np.float32)
...
def calculate_metrics(cfg, results_all):
    ...
    for item_id in tqdm(results_all):
        ...
        contact_error = 0 # min(np.abs(signed_distance).min(), 0.2)
        penetration_error = 0 # max(signed_distance.max(), 0)
...
def evaluate(cfg):
    ...
    for idx, batch in enumerate(tqdm(dataloader)):
        ...
        for idx, img_id in enumerate(batch['img_id']):
            if batch['part_labels'][idx].detach().cpu().numpy() != 4:
                continue
            item_id = img_id + '_' + str(batch['part_labels'][idx].detach().cpu().numpy())
            results_all[item_id] = {
