...
class HOIInstance(nn.Module):
    ...
    def forward(self, ):
        ...
        smpl_v = torch.matmul(smpl_v_centered, hoi_rotmat.permute(0, 2, 1)) + self.hoi_trans.reshape(batch_size, 1, 3)
        smpl_J = torch.matmul(smpl_J_centered, hoi_rotmat.permute(0, 2, 1)) + self.hoi_trans.reshape(batch_size, 1, 3)

        scale = self.object_scale.reshape(-1, 1, 1)
        ...
        object_v = torch.matmul(object_v_org, obj_rotmat.permute(0, 2, 1)) + obj_trans.reshape(batch_size, 1, 3)
        object_v_centered = torch.matmul(object_v_org, obj_rel_rotmat.permute(0, 2, 1)) + self.obj_rel_trans.reshape(batch_size, 1, 3)
        object_kps_centered = torch.matmul(object_kps_org, obj_rel_rotmat.permute(0, 2, 1)) + self.obj_rel_trans.reshape(batch_size, 1, 3)
