...
class Model(nn.Module):
    ...
    def forward(self, batch):
        ...
        pred_gamma = self.stackflow(visual_features, pred_pose6d.reshape(b, -1), batch['part_labels'])
        out = {
    ...
    def inference(self, batch, debug=False):
        ...
        part_labels = batch['part_labels']
        object_anchors_org = batch['object_anchors']
        pred_offsets = self.offset_loss.hooffset.decode(pred_gamma, part_labels)
        pred_obj_rel_R, pred_obj_rel_T = self.offset_loss.hooffset.decode_object_RT(pred_offsets, pred_betas, pred_smpl_rotmat, object_anchors_org, part_labels)
...
class StackFlow(nn.Module):
    ...
    def forward(self, visual_features, theta, object_labels, gamma_z=None):
    ...
    def gaussianize(self, gamma, visual_features, theta, object_labels):
        offset_condition = visual_features
        offset_condition = offset_condition + self.object_embedding(object_labels)
        offset_condition = offset_condition + self.smpl_pose_embedding(theta)
    ...
    def sampling(self, n_samples, visual_features, theta, object_labels, z_std=1.):
        offset_condition = visual_features
        offset_condition = offset_condition + self.object_embedding(object_labels)
        offset_condition = offset_condition + self.smpl_pose_embedding(theta)
    ...
    def log_prob(self, gamma, visual_features, theta, object_labels):
        offset_condition = visual_features
        offset_condition = offset_condition + self.object_embedding(object_labels)
        offset_condition = offset_condition + self.smpl_pose_embedding(theta)
...
class HOOffset(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        state_dict = torch.load('outputs/chairs/chairs_object_inter_models.pth')
        ...
        self.incrementalPCA_all_parts = incrementalPCA_all_parts


    def encode(self, smpl_betas, smpl_body_pose_rotmat, object_rel_rotmat, object_rel_trans, object_anchors_org, part_labels):
        ...
        gamma = []
        for idx, l_ in enumerate(part_labels):
            latent_codes = self.incrementalPCA_all_parts[l_.item()].transform(offsets[idx].unsqueeze(0)).squeeze(0)
            gamma.append(latent_codes)
        gamma = torch.stack(gamma, dim=0)
        return gamma, offsets


    def decode(self, gamma, part_labels):
        offsets = []
        for idx, l_ in enumerate(part_labels):
            offset_ = self.incrementalPCA_all_parts[l_.item()].inverse(gamma[idx].unsqueeze(0)).squeeze(0)
            offsets.append(offset_)
        offsets = torch.stack(offsets, dim=0)
        return offsets


    def decode_object_RT(self, offsets, smpl_betas, smpl_body_pose_rotmat, object_anchors_org, object_labels):
        b = smpl_betas.shape[0]
        smpl_out = self.smpl(body_pose=smpl_body_pose_rotmat, betas=smpl_betas)
        smpl_J = smpl_out.joints
        smpl_v = smpl_out.vertices
...
class OffsetLoss(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        part_labels = targets['part_labels']
        ...
        gt_gamma, gt_offsets = self.hooffset.encode(smpl_betas, smpl_body_pose_rotmat, object_rel_rotmat, object_rel_trans, object_anchors_org, part_labels)
        ...
        pred_offsets = self.hooffset.decode(pred_gamma, part_labels)
        
        pred_obj_rel_R, pred_obj_rel_T = self.hooffset.decode_object_RT(pred_offsets, pred_betas, pred_rotmat, object_anchors_org, part_labels)
...
class FlowLoss(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        gt_gamma, gt_offset = self.hooffset.encode(smpl_betas, smpl_body_pose_rotmat, object_rel_rotmat, object_rel_trans, object_anchors_org, part_labels)
        gt_pose_6d = gt_pose_6d + torch.randn_like(gt_pose_6d) * 0.001
        gt_gamma = gt_gamma + torch.randn_like(gt_gamma) * 0.01
        gamma_log_prob = self.flow.log_prob(gt_gamma, visual_features, gt_pose_6d, part_labels)
